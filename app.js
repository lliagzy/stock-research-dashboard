/* 股票投研看板 · 开源版 前端逻辑
接口契约（同后端）：
  /api/portfolio -> {updated_at, data_source, market, sectors_meta[], watchlist[], featured}
  /api/search?q= -> [{code,name,sector}]
  /api/stock?code= -> detail
  /api/sectors -> {loading:true} | [{name,ret_5d,ret_20d,ret_60d,series[]}]
  /api/sector?name= -> [{code,name,ret60,price,pe,pb}]
*/
...
async function init() {
  initTheme();
  try {
    PORT = await api('/api/portfolio');
  } catch (e) {
    $('detailSection').innerHTML = '<div class="empty">服务未就绪，请先运行 python build.py 与 python app.py</div>';
    return;
  }
  ...
  $('lastRefresh').textContent = PORT.last_refresh || '—';
  $('refreshSched').textContent = (PORT.refresh_schedule || '--:--').replace(' 每日', '');
...
function bindRefresh() {
  ...
  btn.addEventListener('click', async () => {
    ...
    btn.textContent = '刷新中…';
    try {
      const r = await fetch('/api/refresh?scope=all', { method: 'POST' });
      const j = await r.json();
      if (j.status === 'running') {
        $('lastRefresh').textContent = '刷新进行中';
      } else if (j.status === 'done') {
        const s = j.stocks || {};
        $('lastRefresh').textContent = (j.last_refresh || '刚刚');
        // 刷新后重载首屏与新数据
        setTimeout(() => location.reload(), 400);
      } else {
        $('lastRefresh').textContent = j.message || '刷新失败';
        btn.disabled = false;
        btn.textContent = old;
      }
    } catch (e) {
      $('lastRefresh').textContent = '刷新请求失败';
      btn.disabled = false;
      btn.textContent = old;
    }
  });
...
function renderSectorNav(sectors) {
  if (!sectors || !sectors.length) { $('sectorNav').innerHTML = '<div class="muted" style="padding:6px">板块构建中…</div>'; return; }
...
function selectStock(code, name) {
  ...
  api('/api/stock?code=' + code)
    .then(d => { currentDetail = d; renderDetail(d); setActiveWatchlist(code); })
    .catch(e => { $('detailSection').innerHTML = '<div class="empty">加载失败：' + e.message + '</div>'; });
}
...
async function loadSectors() {
  try {
    ...
    if (data && data.loading) {
      $('sectorBar').innerHTML = '<div class="empty">板块指数数据构建中（首次约 1 分钟），请稍后刷新本页…</div>';
      $('sectorTable').innerHTML = '';
      renderSectorNav([]);
      setTimeout(loadSectors, 8000);
      return;
    }
    currentSectors = data;
    drawSectorBar(data);
...
async function openSector(name) {
  ...
  try {
    const data = await api('/api/sectors');
    const sec = (data && !data.loading) ? data.find(s => s.name === name) : null;
    if (sec && sec.series) {
      const tc = themeColors();
    ...
    const cons = await api('/api/sector?name=' + encodeURIComponent(name));
...
function rerenderCharts() {
  ...
  if (charts.secIdx) { /* 重新拉取当前板块索引可省略，下钻时再绘 */ }
}
